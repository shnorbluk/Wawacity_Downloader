...
## Utilisation
### Mode manuel
Pour lancer le programme normalement, lancez ```python main.py``` (Windows) ou ```python3 main.py```(Linux) dans un terminal. Cela lance le programme en mode manuel, c'est à dire que l'utilisation est interactive avec des questions qui vous sont posées tout au long de l'exécution.
...
Cette méthode ouvre le navigateur par défaut, sur l'URL de lien. Vous devez résoudre le CAPTCHA, puis copier-coller le lien de téléchargement.
